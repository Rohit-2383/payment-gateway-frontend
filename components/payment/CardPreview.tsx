...
export function CardPreview({
  ...
  const displayNumber =
    trimmedNumber.length > 0 ? formatCardNumber(trimmedNumber) : PLACEHOLDERS.cardNumber;

  const displayName =
    trimmedName.length > 0 ? trimmedName.toUpperCase() : PLACEHOLDERS.cardholderName;

  const displayExpiry =
    trimmedExpiry.length > 0 ? formatExpiry(trimmedExpiry) : PLACEHOLDERS.expiry;
  ...
  return (
    <Card
      className={cn(
        "relative overflow-hidden border-0 bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-800 text-zinc-50 shadow-sm ring-1 ring-zinc-950/10 dark:ring-zinc-50/10",
        className
      )}
    >
